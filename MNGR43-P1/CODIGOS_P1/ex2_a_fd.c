/*
  ...
  gcc -g -Wall -O3 -o ex2_a_fd ex2_a_fd.c -lm
  Compilar con  sintaxis standard ANSI C del 1990
  gcc -g -Wall -pedantic -o ex2_a_fd ex2_a_fd.c -lm
*/
...
int main (void)
{
    ...
    if(tip==1)
    {
      ...
      xp=(-bf+sqrtf(bf*bf-4*af*cf))/(2*af); //Usamos sqrtf()
    ...
    else if(tip==2)
    {
      ...
      {error(); printf("\n Quizas la ecuacion no tiene soluciones reales");}
      zp=(-bd+sqrt(bd*bd-4*ad*cd))/(2*ad);
      zm=(-bd-sqrt(bd*bd-4*ad*cd))/(2*ad);
      printf( "\n Las soluciones obtenidas a la equacion %0.14G*x^2+%0.14G*x+%0.14G=0",ad,bd,cd);
      printf("\n Son: \n x_+=%.14G  i  \n x_-=%.14G \n",zp,zm);
      }
   else
   {
