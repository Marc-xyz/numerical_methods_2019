...
//Guardamos valores para generar gráfico con gnuplot
       if(plot_dat(n,nodos,poli_coef,outputt,"de Chevyshev")!=0){error(); return -1;}
        n=n*2;
    }
    fclose(output);
    return 0;
...
// No hace falta generar toda la matriz
// Mentira, sí que es necesario 
...
int plot_dat(double n, double nodos[], double poli_coef[], FILE *output, char *tipo)
{
    ...
    for(k=0;k<=180;k++)
        ...
        fprintf(output,"\n\n");
        printf("\n El valor maximo de |f(x_k)-P_f(x_k)| para %g nodos %s es %g\n",n,tipo,max_dif);
    return 0;
}
